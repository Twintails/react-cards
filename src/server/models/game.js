...
export class Game extends RoomBase {
  ...
  constructor(id, title, app) {
    ...
    this.options = {
      scoreLimit: 5,
      cardSets: app.cards.cardSets.map(s => s.id)
    }
  ...
  setOptions(options) {
    ...
    this._tick(() => this.options = {scoreLimit: options.scoreLimit, cardSets: options.cardSets})
    return validator
  }
  ...
  start() {
    this._ensureActive()
    const validator = new Validator()
    validator.assert(this.step === A.STEP_SETUP, "The game is in progress")
    validator.assert(this.players.length >= MINIMUM_PLAYERS, "Too few players")
    validator.assert(this.options.cardSets.length > 0, "Please select a set")
    if (validator.didFail)
      return validator
  ...
  selectStack(stackOrStackId) {
    ...
    if (this.step !== A.STEP_JUDGE_STACKS)
      return Validator.fail("It's not time for that yet")
